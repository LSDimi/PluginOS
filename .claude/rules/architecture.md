...
**Two Execution Paths**: `run_operation` dispatches to pre-built operations (39 registered). `execute_figma` runs arbitrary JS in the plugin sandbox (fallback for custom logic, 5s default / 30s max timeout).
