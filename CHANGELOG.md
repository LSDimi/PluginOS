...
- Added `PLUGINOS_PORT_RANGE` env var to control the port range used by the daemon and its test suites, avoiding collisions with a real running daemon during development.
...
Note: 0.7 → 0.8 is a breaking bump under the 0.x versioning convention. Plugins on 0.7 will show the version-mismatch UI against a 0.8 server — update the Figma bridge plugin together with the MCP server.
